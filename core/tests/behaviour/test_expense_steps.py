...
@when(
    parsers.parse(
        "actualizo el gasto con id {expense_id:d} para que cueste {amount:d} euros"
    )
)
...
@then(
    parsers.parse(
        'el gasto con id {expense_id:d} debe aparecer en el servicio como "{title}"'
    )
)
def check_expense_title_via_service(context, expense_id, title):
    ...
    assert gasto.title == title, (
        f"Se esperaba '{title}' pero se encontró '{gasto.title}'"
    )
